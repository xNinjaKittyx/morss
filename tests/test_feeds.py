...
from morss.feeds import parse, FeedHTML, FeedJSON, FeedXML


def get_feed(url):
    url = f"http://localhost:8888/{url}"
    out = adv_get(url)
...
each_format = pytest.mark.parametrize(
    "url",
    [
        "feed-rss-channel-utf-8.txt",
        "feed-atom-utf-8.txt",
        "feed-atom03-utf-8.txt",
        "feed-json-utf-8.txt",
        "feed-html-utf-8.txt",
    ],
)
