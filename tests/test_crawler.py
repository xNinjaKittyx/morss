...
@pytest.mark.parametrize(
    "url",
    [
        "enc-gb2312-header.txt",
        "enc-gb2312-meta.txt",  #'enc-gb2312-missing.txt',
        "enc-iso-8859-1-header.txt",
        "enc-iso-8859-1-missing.txt",
        "enc-utf-8-header.txt",
    ],
)
def test_encoding_fix_handler(replay_server, opener, url):
    out = adv_get(f"http://localhost:8888/{url}")
