...
from .util import data_path, pkg_path
        ...
        for ruleset in rulesets:
            if "path" in ruleset:
                for path in ruleset["path"]:
                    if fnmatch(url, path):
                        parser = next(
                            [x for x in parsers if x.mode == ruleset.get("mode")]
                        )  # FIXME what if no mode specified?
...
class ParserBase:
    ...
    def convert(self, TargetParser):
        ...
        if type(self) is TargetParser and self.rules == target.rules:
...
class ParserXML(ParserBase):
    ...
    def parse(self, raw):
        parser = etree.XMLParser(
            recover=True, remove_blank_text=True, remove_pis=True
    ...
    def _resolve_ns(self, rule):
        ...
        if match:
            ...
            if match[0] in self.NSMAP:
                return f"{{{self.NSMAP[match[0]]}}}{match[1].lower()}"
    ...
    def rule_set(self, rule, value):
        ...
        html_rich = ("atom" in rule or self.rules.get("mode") == "html") and rule in [
            self.rules.get("item_desc"),
            self.rules.get("item_content"),
        ]
    ...
    def rule_str(self, rule):
        ...
        html_rich = ("atom" in rule or self.mode == "html") and rule in [
            self.rules.get("item_desc"),
            self.rules.get("item_content"),
        ]
...
class ParserHTML(ParserXML):
    ...
    mode = "html"
    mimetype: ClassVar[list[str]] = ["text/html", "application/xhtml+xml"]
...
class ParserJSON(ParserBase):
    ...
    mode = "json"
    mimetype: ClassVar[list[str]] = ["application/json", "application/javascript", "text/javascript"]
    ...
    def tostring(self, encoding="unicode", **k):
        dump = json.dumps(
            self.root, ensure_ascii=False, **k
...
class Feed:
    ...
    title = property(lambda f: f.get("title"), lambda f, x: f.set("title", x), lambda f: f.rmv("title"))
    description = desc = property(lambda f: f.get("desc"), lambda f, x: f.set("desc", x), lambda f: f.rmv("desc"))
    items = property(lambda f: f)
...
class Item:
    ...
    title = property(lambda f: f.get("item_title"), lambda f, x: f.set("item_title", x), lambda f: f.rmv("item_title"))
    link = property(lambda f: f.get("item_link"), lambda f, x: f.set("item_link", x), lambda f: f.rmv("item_link"))
    description = desc = property(
        lambda f: f.get("item_desc"), lambda f, x: f.set("item_desc", x), lambda f: f.rmv("item_desc")
    )
    content = property(
        lambda f: f.get("item_content"), lambda f, x: f.set("item_content", x), lambda f: f.rmv("item_content")
    )
    time = property(
